...
<p align="center">
  <strong>Claude Code, Codex, Antigravity 사용량을 macOS 메뉴 막대에서 추적합니다.</strong>
</p>
...
<p align="center">
  <a href="#소스에서-설치"><strong>macOS 앱 빌드</strong></a>
  ·
...
<p align="center">
  <em>v1.18.2는 긴 Rich quota card title이 레이아웃을 밀어내지 않도록 고치고 ellipsis 표시를 유지합니다.</em>
</p>

<p align="center">
  Claude Code, Codex, Antigravity의 토큰, 비용, 세션, 캐시, 모델별 사용량, quota를 한눈에 보여주는 로컬 우선 macOS 메뉴 막대 앱입니다.
</p>
...
이 브랜치는 macOS 포팅 버전입니다. 로컬에서 `.app`, `.dmg`, `.zip`을 빌드합니다.
...
빌드 또는 설치 시 [최종 사용자 라이선스 계약 (EULA)](EULA.ko.txt)에 동의하는 것으로 간주됩니다.
...
- **Trend 카드** — 일/주/월 cost/token 히스토리와 git 순 라인 산출을 함께 표시하며, 일부 데이터가 비어도 0처럼 이어 그리지 않음
...
- **메뉴 막대 라벨** — 사용량 %, 토큰 수, 비용 직접 표시
- **프로젝트 관리** — 숨기기 또는 추적에서 완전 제외
- **로그인 시 자동 실행** — 선택적 자동 실행
...
### 1. 대시보드 열기
메뉴 막대 아이콘 클릭 (또는 전역 단축키 `Ctrl+Shift+D`).
...
WhereMyTokens는 local-first Electron 메뉴 막대 앱입니다. renderer는 로컬 파일이나 자격 증명을 직접 읽지 않으며, 파일 시스템, provider API, 메뉴 막대, 설정 작업은 Electron main process에서 처리하고 preload bridge를 통해서만 renderer에 전달합니다.

| 계층 | 역할 |
|------|------|
| Electron main | Provider 세션 발견, JSONL/local RPC 사용량 조회, 메뉴 막대/창 상태 관리, 앱 설정 저장. |
| Preload bridge | `contextIsolation` 경계를 유지하면서 typed `window.wmt` IPC 표면만 노출. |
| React renderer | 메뉴 막대 대시보드, 설정, 알림, 활동 차트, compact quota 위젯 표시. |
...
| Claude 브리지 | Claude Code `statusLine` stdin | `~/Library/Application Support/WhereMyTokens/live-session.json` | 없음 |

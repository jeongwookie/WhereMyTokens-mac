...
이 저장소는 macOS 포팅 작업 공간입니다. 기존 Windows 앱은 `main`에 baseline으로만 import했고, 실제 macOS 작업은 `codex/dev-macos-port`와 하위 작업 브랜치에서 진행합니다. 최종 리뷰 전에는 구현 변경을 `main`에 머지하지 않습니다.
...
| 버전 | 날짜 | 주요 변경 |
|------|------|-----------|
| **[v1.18.2](https://github.com/jeongwookie/WhereMyTokens/releases/tag/v1.18.2)** | 6/5 | 긴 Rich quota card title이 Plan Usage 컬럼을 밀어내지 않도록 수정하고 ellipsis와 tooltip fallback을 유지. |
...
| **[v1.18.0](https://github.com/jeongwookie/WhereMyTokens/releases/tag/v1.18.0)** | 6/2 | local-only Antigravity provider 지원, 프로세스 발견, local RPC quota/session scan, 지속 usage cache, provider ledger import 추가. |
| **[v1.17.0](https://github.com/jeongwookie/WhereMyTokens/releases/tag/v1.17.0)** | 6/2 | Provider quota snapshot 기반으로 Plan Usage를 재구성하고 target별 표시 그룹과 quota 상태 마이그레이션을 보강. |
| **[v1.16.1](https://github.com/jeongwookie/WhereMyTokens/releases/tag/v1.16.1)** | 5/27 | 잘린 full-history import나 실패한 source가 있어도 budgeted ledger warmup을 계속하고 stale provider completion marker를 방지. |
...
이 포트는 기존 desktop tray shell을 단순히 다시 포장한 것이 아닙니다.
...
| `~/.claude/.credentials.json` | Anthropic 사용량 요청과 token refresh용 Claude OAuth 정보. |
...
- 최종 owner review 전 `main`에 구현 merge가 없는지 확인.
